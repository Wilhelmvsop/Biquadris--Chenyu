...
export class Board {
   ...
   public:
    ...
    // element: vector of blocks cleared (user don't need to delete because its
    // unique_ptr)
    std::tuple<bool, int, std::vector<std::shared_ptr<Block>>> drop() noexcept;

    // Setters and getters
    bool setCurrentBlock(std::shared_ptr<Block>
                             newBlock) noexcept;  // only called ONCE in the
                                                  // beginning, modifies canvas
