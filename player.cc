...
export class Player {
    ...
    int score;
    int highscore;
    std::shared_ptr<std::istream> input;  // source when prompt something back to real player

    int calculateRowScore(int numRowsCleared) const;
    // calculate scores from blocksCleared and delete them
    int calculateBlockScore(const std::vector<std::shared_ptr<Block>>& blocksCleared) const;

   public:
    // take the target level and set up a new board (player owns the board)
    Player(std::shared_ptr<Level> level, std::shared_ptr<std::istream> input = nullptr);
    ~Player();
    ...
    std::vector<std::vector<char>> getPixels(const Debuff& debuff = {}) const;
