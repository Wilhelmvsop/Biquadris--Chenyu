...
TEST_CASE(InputHandler_resolve)
{ // resolve checks: exact matches
	for (auto &p : aliases)
	{
		REQUIRE_EQUAL(p.second, ih.resolve(p.first));
		// Unique-prefix resolution checks
		// "levelup" vs "leveldown" -> "lev" ambiguous, specific prefixes should resolve
		...
		// multiplier: prefix digits should be accepted for commands that allow multipliers
